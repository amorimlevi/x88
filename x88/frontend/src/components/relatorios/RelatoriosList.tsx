...
const RelatoriosList = () => {
  ...
  // Listener para atualizações em tempo real dos relatórios
  useEffect(() => {
    // Limpar dados iniciais para ver modificações em tempo real
    relatoriosService.limparDados()
    
    const unsubscribe = relatoriosService.addListener(() => {
  ...
  useEffect(() => {
    loadReportData()
  }, [filtroTempo, tipoRelatorio, startDate, endDate, updateTrigger])
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      // Incluir dados do novo serviço de relatórios
      const novosRelatorios = relatoriosService.filtrarPorPeriodo(startDate, endDate)
      const dadosConvertidos = novosRelatorios.map(item => ({
        id: item.id,
        funcionarioId: item.funcionario.split(' ')[0].toLowerCase(),
        funcionarioNome: item.funcionario,
        tipo: item.tipo as 'salario' | 'adiantamento' | 'viagem' | 'bonus',
        valor: item.valor,
        descricao: item.detalhes,
        status: item.status as 'pago' | 'pendente' | 'agendado' | 'cancelado',
        dataPagamento: `${item.data}T12:00:00`,
        metodoPagamento: 'sistema' as const,
        origem: 'sistema' as const,
        observacoes: item.observacoes
      }))
      
      setPagamentos([...data, ...dadosConvertidos])
    } catch (error) {
